...
let MailService = class MailService {
    ...
    async sendVerificationOTP(to, otp) {
        ...
        const otpDigits = otp.split('').map(d => `<td style="padding:0 6px;"><div style="width:52px;height:64px;line-height:64px;background:#f0f2ff;border:2px solid #667eea;border-radius:12px;font-size:32px;font-weight:800;color:#667eea;text-align:center;font-family:'Segoe UI',Arial,sans-serif;">${d}</div></td>`).join('');
        const html = `
<!DOCTYPE html>
<html lang="en">
...
<body style="margin:0;padding:0;background-color:#f4f6fb;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f6fb;padding:40px 0;">
    ...
    async sendPasswordResetOTP(to, otp) {
        ...
        const otpDigits = otp.split('').map(d => `<td style="padding:0 6px;"><div style="width:52px;height:64px;line-height:64px;background:#fff0f3;border:2px solid #f5576c;border-radius:12px;font-size:32px;font-weight:800;color:#f5576c;text-align:center;font-family:'Segoe UI',Arial,sans-serif;">${d}</div></td>`).join('');
        const html = `
<!DOCTYPE html>
<html lang="en">
